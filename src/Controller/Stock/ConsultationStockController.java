...
public class ConsultationStockController implements Initializable {
    ...
    private void recherchMouseClicked(MouseEvent event) throws ParseException {
          ...
          if (txtCode.getText().equalsIgnoreCase("") && dateOper.getValue() == null)
        {
        nav.showAlert(Alert.AlertType.ERROR, "Alert", null, Constantes.CHAMP_OBLIGATOIRE);
        
        }else if (txtCode.getText().equalsIgnoreCase("") && dateOper.getValue() != null ){

              LocalDate localDate=dateOper.getValue();
            
                Date dateSaisieOper=new SimpleDateFormat("yyyy-MM-dd").parse(localDate.toString());
            
            List<SituationStock> listSituationStock = situationStockDAO.findDateBySituationStock(dateSaisieOper);
   ...
   tableSituationStock.setItems(listeSituationStock);
    setColumnProperties();
               ...
               List<SituationStock> situationStock = situationStockDAO.findDateCodeArtListBySituationStock(dateSaisieOper, txtCode.getText());
